 ...
 * @param filename The name if the file to display this image
 */
...
void Item::Draw(wxDC* dc)
{
    double wid = mItemBitmap->GetWidth();
    double hit = mItemBitmap->GetHeight();
    dc->DrawBitmap(*mItemBitmap, int(GetX()-wid/2), int(GetY()-hit/2));
}
...
/**
    ...
    * @param m New mirror image
    */
void Item::SetMirror(bool m)
{
    if (m) {
        //  This code only executes if the mirror changes
        mMirror = m;
